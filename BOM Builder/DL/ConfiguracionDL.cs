...
namespace BOM_Builder.DL
{
  public class ConfiguracionDL : DataAccessSQL
  {
    ...
    public List<NM_ModelosLModel> GetListModelL1(int id, out string errorMethod)
    {
      ...
      try
      {
        using (var conn = new SqlConnection(connectionSQL))
        using (var cmd = conn.CreateCommand())
        {
          cmd.CommandText = @"SELECT Id, Nombre_Modelo, Descripcion_Modelo, 
                                Aplica_SubNivel, Mfg_Cell 
                                FROM NM_ModeloL1 
                                WHERE Id_ModeloL0 = @Id_ModeloL0";
          cmd.CommandType = CommandType.Text;
          cmd.Parameters.Add("@Id_ModeloL0", SqlDbType.Int).Value = id;
          ...
          using (SqlDataReader data_reader = cmd.ExecuteReader())
          {
            while (data_reader.Read())
            {
              var data_model = new NM_ModelosLModel
              {
                ...
                Secuence = GetSecuence(Convert.ToInt32(data_reader["Id"]))
              };
              data_list.Add(data_model);
            }
          }
    ...
    private string GetSecuence(int nmModeloId)
    {
      using (var conn = new SqlConnection(connectionSQL))
      using (var cmd = conn.CreateCommand())
      {
        cmd.CommandText = @"SELECT descripcion FROM NM_Secuencias WHERE id = @nmModeloId";
        cmd.CommandType = CommandType.Text;
        cmd.Parameters.Add("@nmModeloId", SqlDbType.Int).Value = nmModeloId;
        conn.Open();
        return NotNullHelper.NotNullString(cmd.ExecuteScalar());
      }
    }
